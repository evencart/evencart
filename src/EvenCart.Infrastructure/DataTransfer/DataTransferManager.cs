...
namespace EvenCart.Infrastructure.DataTransfer
{
    public class DataTransferManager : IDataTransferManager
    {
        ...
        private int ImportProducts(DataTransferChunk chunk, IDataTransferProvider dataTransferProvider)
        {
            var done = 0;
            try
            {
                var products = dataTransferProvider.GetProducts(chunk);
        ...
        private int ImportUsers(DataTransferChunk chunk, IDataTransferProvider dataTransferProvider)
        {
            var done = 0;
            try
            {
                var users = dataTransferProvider.GetUsers(chunk);
                //saved roles
        ...
        private int ImportCategories(DataTransferChunk chunk, IDataTransferProvider dataTransferProvider)
        {
            var done = 0;
            try
            {
                var categories = dataTransferProvider.GetCategories(chunk);
                categories.ForEach(category =>
                {
                    category.Id = 0;
                    var parent = category.Parent;
        ...
        public int Import<T>(DataTransferChunk chunk, IDataTransferProvider dataTransferProvider) where T : FoundationEntity
        {
            switch (typeof(T).Name)
            {
                case nameof(Product):
                    return ImportProducts(chunk, dataTransferProvider);
                case nameof(Category):
                    return ImportCategories(chunk, dataTransferProvider);
                case nameof(User):
                    return ImportUsers(chunk, dataTransferProvider);
            }

            return 0;
        }

        public DataTransferChunk Export<T>(IDataTransferProvider dataTransferProvider) where T : FoundationEntity
        {
            ...
            switch (typeof(T).Name)
            {
                case nameof(Product):
                    ...
                    chunk = dataTransferProvider.GetTransferChunks(products);
                    break;
                case nameof(Category):
                    var categories = _categoryService.Get(x => true).ToList();
                    chunk = dataTransferProvider.GetTransferChunks(categories);
                    break;
                case nameof(User):
                    var exceptRoleIds = new[]
                        {_roleService.FirstOrDefault(x => x.SystemName == SystemRoleNames.Visitor)?.Id ?? 0};
                    var users = _userService.GetUsers(null, exceptRoleIds, x => x.Id, SortOrder.Ascending, 1,
                        int.MaxValue, out _, negateRoleRestriction: true);

                    chunk = dataTransferProvider.GetTransferChunks(users);
                    break;
            }
