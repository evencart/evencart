...
namespace Genesis.ViewEngines.GlobalObjects
{
    public class CartObject : GlobalObject
    {
        ...
        public override object GetObject()
        {
            ...
            var taxSettings = D.Resolve<TaxSettings>();
            var priceAccountant = D.Resolve<IPriceAccountant>();
            var dataSerializer = D.Resolve<IDataSerializer>();

            if (D.Resolve<IGenesisEngine>().CurrentUser == null)
                return null;
            var cart = _isWishList
                ? cartService.GetWishlist(D.Resolve<IGenesisEngine>().CurrentUser.Id)
                : cartService.GetCart(D.Resolve<IGenesisEngine>().CurrentUser.Id);
            ...
            var currentCurrency = D.Resolve<IGenesisEngine>().CurrentCurrency;
            cartModel.Items = cart.CartItems.Select(x =>
                {
                    var cartItem = new CartItemImplementation {
                        ...
                        ImageUrl = mediaAccountant.GetPictureUrl(x.Product.MediaItems?.FirstOrDefault(), D.Resolve<IGenesisEngine>().ActiveTheme.CartItemImageSize, true),
